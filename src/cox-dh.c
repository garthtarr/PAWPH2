 ...
 *
 * Modification: cdfit_cox_dh() accepts a per-observation intercept vector
 * (intercept_) that is incorporated into the initial eta at the start of each
 * coordinate descent path. This enables the PAWPH outlier detection mechanism.
 */
...
// Coordinate descent for Cox models (modified to accept per-observation intercepts)
SEXP cdfit_cox_dh(SEXP X_, SEXP d_, SEXP penalty_, SEXP lambda, SEXP eps_, SEXP max_iter_, SEXP gamma_, SEXP multiplier, SEXP alpha_, SEXP dfmax_, SEXP user_, SEXP warn_,
  ...
  double *intercept=REAL(intercept_);
  ...
  for (int i=0; i<n; i++) eta[i] = intercept[i];
      ...
      while (tot_iter < max_iter) {
        ...
        for (int j=0; j<p; j++) {
          if (e[j]) {
            ...
            // Update b_j
            l1 = lam[l] * m[j] * alpha[j];
            l2 = lam[l] * m[j] * (1-alpha[j]);
