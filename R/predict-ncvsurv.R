...
#
# Function: predict.ncvsurv()
# S3 prediction method for ncvsurv objects.

#' @method predict ncvsurv
#' @export
#' @keywords internal
predict.ncvsurv <- function(object, X,
  ...
  for (j in 1:ncol(eta)) {
    ...
    for (i in 1:nrow(eta)) {
      S <- S0^exp(eta[i, j])
      if (type == 'survival') val[[i]] <- stats::approxfun(x, S, method='constant', ties=function(x) tail(x, 1))
      if (type == 'median') {
        if (any(S < 0.5)) {
